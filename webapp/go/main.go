...
func NewMySQLConnectionEnv() *MySQLConnectionEnv {
	return &MySQLConnectionEnv{
		Host:     getEnv("MYSQL_HOST", "10.163.99.103"),
...
func getLowPricedChair(c echo.Context) error {
	var chairs []Chair
	query := `SELECT * FROM (SELECT * FROM chair WHERE stock > 0  LIMIT ?) as ch ORDER BY price ASC, id ASC`
...
func searchRecommendedEstateWithChair(c echo.Context) error {
	...
	h := chair.Height
	d := chair.Depth
	query = `SELECT * FROM (SELECT * FROM estate WHERE (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?)) as ch ORDER BY popularity DESC, id ASC`
